let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    conn.sendFile(m.chat, img, 'SGDC.png', '*[ • SGDC-BOT • ]', m)
  } finally {
    delete conn.hartatahta[m.chat]
  }
}

handler.command = /^(sgdc2)$/i
handler.limit = true
...
function ht(text = '') {
  return new Promise((resolve, reject) => {
    ...
    let layers = [
      ...
      textArgs('• MEMBER •', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2-(text_h*${lh})`, w, h) + format + '[top]',
