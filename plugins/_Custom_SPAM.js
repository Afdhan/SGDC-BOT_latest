...
let [kr, psn] = text.split `|`

    if (!kr) return conn.reply(m.chat, 'Siapa yang mau di santet?', m)
 ...
 await m.reply('_Proses Pengiriman 100 Spam..._')
    let kntl = `*Berhasil mengirim Spam ke nomor tujuan!*`
    conn.reply(m.chat, kntl, m)

}

handler.command = /^(spaming)$/i
