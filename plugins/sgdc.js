let handler = async (m, { conn, text }) => {
 ...
 // if (err) return reply('Gagal! Terjadi kesalahan :)')
  else conn.hartatahta[m.chat] = true
  m.reply('Sedang membuat... Mohon tunggu sekitar 1 - 2 menit')
  try {
    let img = await ht(text ? text : 'SGDC-BOT')
...
function ht(text = '') {
  return new Promise((resolve, reject) => {
    ...
    let layers = [
      ...
      textArgs('GRETONGAN', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2-(text_h*${lh})`, w, h) + format + '[top]',
      textArgs('SGDC-TEAM', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2`, w, h) + format + '[mid]',
